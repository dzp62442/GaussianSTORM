...
掩码语义统一为 **True/白色=有效，False/黑色=排除**。参考项目中名为 `valid_depth_mask` 的变量有一次取反，不能按变量名照搬。单路乃至十二路新视角的 mask 全黑是允许的：这些视角仍保留在 18 路样本中，RGB/深度损失不计其被屏蔽像素；LPIPS 的预测与真值均置零，该视角贡献为零，仍按原 18 路求均值。末尾六路输入目标保持全有效。若某个损失项没有可用监督像素，该项返回具有零梯度的零值，其他项照常计算；单路或全部 Metric3D 不含有效深度均不在加载阶段拒绝。不得把“全屏蔽”当成缺文件，也不改为全图监督。真实无法读取的文件、接口形状错误及模型实际产生的非有限训练损失仍明确报告，不把数值训练故障归因于数据集。
...
### 3.4 数据检查边界自查（2026-09-25）

沿 OmniScene 加载、损失、评估汇总和日志链路自查，取消适配时增加的下列数据数值拒绝条件。正常输入上的公式保持不变，不改数据、不生成新信息、不跳过 bin，也不把未定义指标替换成正常分数。

| 情况 | 当前处理 |
| --- | --- |
| 单路或多路 mask 全黑 | 保留该视角和样本；该区域不参与监督 |
| 某路 Metric3D 全零或没有有限正深度 | 正常加载；由深度损失已有有效域选择像素；整个损失项有效域为空则该项为零 |
| DA2 常量图、零值或非有限数值 | 使用与参考项目相同的转换公式，不以数值检查拒绝；公式未定义的结果传至 PCC 统计 |
| 相机数值 | 移除额外的正焦距、旋转子矩阵行列式阈值和加载阶段非有限数值检查；保留接口要求的矩阵形状，按原值交给模型 |
| 清单出现重复 token | 按原清单的顺序及出现次数使用；不自行去重或拒绝，评估覆盖核对仍要求与清单逐项一致 |
| PCC 方差为零或输入非有限 | 该组该 bin 的 PCC 记为 `null`，记录原因；其他指标和后续样本继续计算 |
| PSNR 正无穷或其他非有限指标 | JSON 用 `"Infinity"`/`"-Infinity"` 表示无穷、`null` 表示未定义，不因 JSON 写入或格式化失败终止 |

每个 bin 保留记录；某组某项有任何 `null`，该组整集均值也为 `null`，不静默只平均部分 bin。`summary.json` 的 `complete` 表示样本遍历完成，`metrics_defined` 表示所有整集指标有定义；另存 `undefined_metric_counts` 和 `metric_issues.json`（bin、视角组、原因）。两者独立，未定义 PCC 不阻断训练中评估调度或最终测试完成标记。W&B 只写可用数值，飞书显示“未定义”或“+∞”并附计数。

保留真正的接口/运行要求：文件可读取、必要相机条目存在、张量形状匹配、固定六输入/十八目标和静态时间协议、恢复配置与 checkpoint 一致、模型训练数值可用于更新。没有任意替换相机参数、跳过文件读取错误或把 NaN 梯度写入模型的兜底。这些是计算边界，不作为对数据集质量的推断。
...
转换沿用参考函数；常量参考图等导致公式未定义时不在加载阶段终止，PCC 记录为未定义并报告 bin/视角组，训练继续。不静默填零，不调用 DA2 模型补算，不删除样本或仅平均可计算样本。真实文件读取失败仍保留明确的路径错误。
...
所有主指标均使用完整图像范围，不以训练动态掩码筛像素；不引入其他数据集的 ego mask。深度背景的零值保留，不因 alpha 小就额外过滤。若出现非有限值、有效元素不足或任一向量方差为零，该 PCC 及对应整集 PCC 标为 `null`，记录原因并继续评估及训练；不改为零、不静默忽略后声称指标完整。正常样本的计算公式和双组统计方式不变。
...
metric_issues.json      # 指标未定义/无穷的 bin、视角组和原因；不删除记录
...
| 自动续训 CPU 回归 | 2 项：两个分辨率默认启用、独立测试兼容、当前实验隔离、忽略临时文件、自动选择与显式路径优先级；原故障恢复测试改为不传恢复路径，并加入随机前向，验证恢复后结果与连续训练一致，已完成实验不重写最终 checkpoint |
| 全屏蔽视角 CPU 回归 | 2 项：十二路新视角全黑 mask 仍保留全部目标、缺失文件继续报错、RGB/深度/LPIPS 的屏蔽区域梯度为零、LPIPS 保持原 18 路均值；真实故障 bin `scene881da81bb35a4bdb925e0a2884ee7f0f_bin005` 在两种分辨率的 RGB/mask 与 depthsplat 原函数一致，并以合成预测通过原生 VGG LPIPS 与深度损失反向检查（CPU，无完整模型训练） |
| 数据数值边界 CPU 回归 | 新增 4 项，合计 28 项通过：零深度/常量 DA2/重复清单可加载、相机无额外数值阈值、空监督零梯度、未定义指标不删样本且保留整集语义、评估完成及诊断 JSON 写入、飞书格式化；已有测试同步覆盖正常输入结果不变 |
